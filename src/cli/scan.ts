...
export async function scanCommand(options: { check?: boolean }): Promise<void> {
  ...
  if (options.check) {
    ...
    const { store: fresh, complete } = await buildAnatomy(wolfDir, projectRoot);
    if (!complete) { console.error("Cannot certify freshness: anatomy scan is incomplete."); process.exitCode = 1; return; }
    const newContent = renderStore(buildMergedStore(wolfDir, projectRoot, fresh, complete));
  ...
  console.log(`  ✓ Anatomy scan visited: ${fileCount} files indexed in ${elapsed}ms`);
}
